...
export default function DirectoryLandingPage({ config }: Props) {
  ...
  return (
    <div className="min-h-screen bg-[#1C3A22] text-slate-50">
      {/* Nav */}
      <nav className="sticky top-0 z-20 border-b border-white/5 bg-[#1C3A22]/90 backdrop-blur">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <select
                    ...
                    className="w-full rounded-lg border border-white/10 bg-[#1C3A22] px-3 py-2 text-sm text-slate-50 focus:border-white/20 focus:outline-none focus:ring-1"
                    style={{ "--tw-ring-color": accentColor } as React.CSSProperties}
                  >
                    {pricingTiers.map((t) => (
