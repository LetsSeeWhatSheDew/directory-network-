...
export default function VerifiedRow({
  ...
  const sage = "var(--pp-signal)";
  const sageVibrant = "var(--pp-canopy-eyebrow)";
  ...
  return (
    <span style={baseStyle} aria-label={`${verb}${pct != null ? ` ${pct}% off` : ""}${time ? ` at ${time}` : ""}`}>
      ...
      <span>
        {verb}
        {pct != null ? <strong style={{ fontWeight: 700, color: stale ? labelColor : tone === "dark" ? "var(--pp-canopy-text)" : "var(--pp-ink)" }}>{` ${pct}%`}</strong> : null}
