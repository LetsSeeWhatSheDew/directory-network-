...
export default function ZipSignup({ source, cta, channel }: { source: "delivery" | "drive_thru"; cta: string; channel?: string }) {
  ...
  const field: React.CSSProperties = { padding: "11px 12px", borderRadius: 10, border: "1px solid var(--pp-border)", fontSize: 15, background: "#fff", color: "var(--pp-ink)", minWidth: 0 };
  ...
  return (
    ...
    >
      ...
      <button type="submit" disabled={state === "busy"} style={{ padding: "11px 16px", borderRadius: 10, border: "none", fontWeight: 700, fontSize: 15, cursor: "pointer", background: "var(--pp-paper)", color: "var(--pp-canopy)", flex: "1 1 auto" }}>
