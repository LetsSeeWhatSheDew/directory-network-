...
export default function ZipSignup({ source, cta, channel }: { source: "delivery" | "drive_thru"; cta: string; channel?: string }) {
  ...
  const field: React.CSSProperties = { padding: "12px 14px", borderRadius: 14, border: "1px solid var(--pp-border)", fontSize: 15, background: "var(--pp-surface)", color: "var(--pp-ink)", minWidth: 0 };
  ...
  return (
    ...
    >
      ...
      <button type="submit" disabled={state === "busy"} style={{ padding: "12px 18px", borderRadius: 14, border: "1px solid var(--pp-btn-border)", fontWeight: 600, fontSize: 15, cursor: "pointer", background: "var(--pp-btn)", color: "var(--pp-btn-fg)", flex: "1 1 auto" }}>
