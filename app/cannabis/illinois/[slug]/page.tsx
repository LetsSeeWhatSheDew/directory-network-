...
async function getCityListings(city: string): Promise<Listing[]> {
  return fetchJson<Listing[]>(
    `/master_listings?city=ilike.${encodeURIComponent(city)}&state=eq.IL&select=id,name,slug,city,state,address1,phone,website,short_description,logo_url,plan,claimed,delivery,online_ordering,atm_onsite,wheelchair_accessible,type&order=plan.desc,name.asc&limit=50`
  );
}
...
export default async function CityPage({
  ...
  const schemaOrg = JSON.stringify({
    ...
    url: `https://cleanlist.co/cannabis/illinois/${citySlug}`,
