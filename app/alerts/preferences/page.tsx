    ...
    <>
      <style>{`
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .sub{font-size:.9rem;color:var(--pp-muted);font-family:system-ui,sans-serif;margin-bottom:28px}
        form{background:var(--pp-surface);border:1px solid var(--pp-border);border-radius:14px;padding:24px}
        .field{margin-bottom:20px}
        label.field-label{display:block;font-size:.8rem;font-weight:600;color:var(--pp-ink);font-family:system-ui,sans-serif;margin-bottom:8px;text-transform:uppercase;letter-spacing:.05em}
        input[type=email],input[type=text]{width:100%;padding:10px 12px;border:1px solid var(--pp-border);border-radius:8px;font-family:system-ui,sans-serif;font-size:.92rem;color:var(--pp-ink);outline:none}
        input[type=email]:focus,input[type=text]:focus{border-color:var(--pp-signal-fill)}
        .radio-row,.check-row{display:flex;gap:8px;flex-wrap:wrap}
        .pill{display:inline-flex;align-items:center;gap:8px;padding:8px 14px;border:1px solid var(--pp-border);border-radius:100px;font-family:system-ui,sans-serif;font-size:.85rem;cursor:pointer;background:var(--pp-surface);color:var(--pp-body);transition:all .15s}
        .pill input{accent-color:var(--pp-signal);margin:0}
        .pill:hover{border-color:var(--pp-muted)}
        .pill.active{background:var(--pp-best-tint);border-color:var(--pp-signal-fill);color:var(--pp-signal);font-weight:600}
        .freq{display:grid;grid-template-columns:1fr;gap:8px}
        .freq label{display:flex;align-items:flex-start;gap:10px;padding:12px 14px;border:1px solid var(--pp-border);border-radius:10px;cursor:pointer;font-family:system-ui,sans-serif;font-size:.88rem;color:var(--pp-body);background:var(--pp-surface);transition:all .15s}
        .freq label.active{background:var(--pp-best-tint);border-color:var(--pp-signal-fill);color:var(--pp-signal)}
        .freq label input{margin-top:3px;accent-color:var(--pp-signal)}
        .freq-title{font-weight:600;display:block}
        .freq-desc{font-size:.78rem;color:var(--pp-muted);margin-top:2px}
        .freq label.active .freq-desc{color:var(--pp-signal)}
        .save{background:var(--pp-signal-fill);color:var(--pp-on-dark);border:none;border-radius:10px;padding:12px 24px;font-family:system-ui,sans-serif;font-weight:700;font-size:.95rem;cursor:pointer;width:100%;margin-top:8px;transition:background .15s}
          ...
          <div className="field">
            ...
            <div className="freq">
              {[
                ...
                { v: "sms", t: "Instant alerts (Pro)", d: "$0.99/mo · email + browser alert the moment a deal goes live" },
              ].map((f) => (
