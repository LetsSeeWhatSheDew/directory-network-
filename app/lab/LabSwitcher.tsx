...
export default function LabSwitcher({ current }: { current: string }) {
  return (
    <nav aria-label="Design previews" style={{ position: "fixed", left: "50%", bottom: 14, transform: "translateX(-50%)", zIndex: 1000, display: "flex", gap: 4, padding: 4, borderRadius: 999, background: "rgba(20,20,20,.86)", backdropFilter: "blur(8px)", boxShadow: "0 6px 24px rgba(0,0,0,.25)", fontFamily: "system-ui,sans-serif" }}>
      {DIRS.map(([k, label]) => (
