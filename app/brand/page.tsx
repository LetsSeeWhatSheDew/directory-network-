    ...
    <>
      <style>{`
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .eyebrow{font-family:system-ui,sans-serif;font-size:.72rem;font-weight:700;letter-spacing:.14em;text-transform:uppercase;color:#2E7D32;margin-bottom:14px}
      ...
      <div className="wrap">
        ...
        {brands.length === 0 ? (
          ...
          <ul style={{ listStyle: "none", display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 12 }}>
            {brands.map((b) => (
              <li key={b.slug}>
                <Link href={`/brand/${b.slug}`} style={{ display: "block", padding: 14, background: "#fff", border: "1px solid #DCDED2", borderRadius: 10, textDecoration: "none", color: "#1F3D2B", fontFamily: "system-ui, sans-serif", fontWeight: 600 }}>
                  {b.name}
                </Link>
              </li>
            ))}
