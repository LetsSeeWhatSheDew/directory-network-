// Debug helper: print ways-to-buy lines (drive-thru, curbside, medical, order-ahead) from rendered pages.
...
(async () => {
  ...
  for (const u of process.argv.slice(2)) {
    ...
    try {
      ...
      const lines = texts.join("\n").split("\n").map((s) => s.trim()).filter((s) => /drive[ -]?(thru|through)|curbside|medical|order ahead|pre-?order|express pick|delivery|hours|close/i.test(s) && s.length < 160);
